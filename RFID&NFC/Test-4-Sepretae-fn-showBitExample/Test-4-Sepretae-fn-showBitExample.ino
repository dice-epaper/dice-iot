...
void loop() {
  ...
  // tagging Card ID가 1번이면
    ...
    showFont("선미선미선미선미선미선미선미선미선미", u8g2_font_unifont_t_korean1, 10, 20);
    //showNum(isFirstTag, u8g2_font_unifont_t_korean1,10,40);
    showFont("100g당 2400원", u8g2_font_unifont_t_korean1,10,40);
    if(isFirstTag == 1){
        showfreeFont("10,000",&FreeSans9pt7b,80,55);
      }
    else{
        showfreeFont("10,000",&FreeSans9pt7b,80,62);
      }

    showfreeFontSale("8,000",&FreeSansBold18pt7b,80,90,true);
    showfreeFont("19.08.25~19.08.26", &Org_01, 120,100);
    display.update();
  }
  ...
  // tagging Card ID가 1번이면
    ...
    showFont("민주민주민주[Dole] 바나나 2송이 250g", u8g2_font_unifont_t_korean1,10,20);
...
void showfreeFontSale(const char name[], const GFXfont* f,const int num1, const int num2, boolean color)
{
  display.setRotation(1);
  if(color){
