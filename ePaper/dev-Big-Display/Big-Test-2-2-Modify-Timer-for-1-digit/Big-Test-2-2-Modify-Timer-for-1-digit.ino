...
#include <GxIO/GxIO.h>
#if defined(ESP8266)
GxIO_Class io(SPI, /*CS=D8*/ SS, /*DC=D3*/ 0, /*RST=TX*/ 1); 
GxEPD_Class display(io, /*RST=TX*/ 1, /*BUSY=D2*/ 4); 
#endif
...
void setup()
{
  ...
  user_stime.tm_mday   = 28;
  user_stime.tm_hour   = 15;
  user_stime.tm_min    = 42;
  user_stime.tm_sec    = 0;
...
void loop()
{
  ...
  // time Sale (시간 변화 적용)
    ...
    for(int i= remainMin; i>0; i--){
      myDFPlayer.play(1);
      firstscreen_timesale(String(i).c_str());
      display.update(); 
...
void firstscreen_timesale(const char min[])
{
  display.fillRect(0, 0, 400, 150, GxEPD_RED);
